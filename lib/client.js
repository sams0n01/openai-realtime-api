...
export class RealtimeClient extends RealtimeEventHandler {
  ...
  constructor({ url, apiKey, dangerouslyAllowAPIKeyInBrowser, debug } = {}) {
    super();
    // GA format mapping: beta names → MIME types
    this._audioFormatMap = {
      'pcm16': 'audio/pcm',
  ...
  _resetConfig() {
    ...
    this.lastCommittedAudioBuffer = new Int16Array(0);
    return true;
  }
  ...
  _addAPIEventHandlers() {
    ...
    this.realtime.on('server.input_audio_buffer.speech_stopped', (event) => {
      console.log('[GA DEBUG] speech_stopped fired, item_id:', event.item_id, 'inputAudioBuffer length:', this.inputAudioBuffer?.length);
      const { item, delta } = this.conversation.processEvent(event, this.inputAudioBuffer);
      console.log('[GA DEBUG] speech_stopped processed, item returned:', !!item, 'audio length:', item?.formatted?.audio?.length);
      // GA API fix: if the item already existed and audio was retroactively set,
      // dispatch conversation.updated so the save handler can pick it up
      if (item) {
        this.dispatch('conversation.updated', { item, delta });
      }
    });
    ...
    this.realtime.on('server.conversation.item.created', (event) => {
      console.log('[GA DEBUG] conversation.item.created, id:', event.item?.id, 'role:', event.item?.role, 'type:', event.item?.type);
      const { item } = handlerWithDispatch(event);
      console.log('[GA DEBUG] item created, audio length:', item?.formatted?.audio?.length, 'status:', item?.status);
    ...
    this.realtime.on(
      ...
      (event) => {
        console.log('[GA DEBUG] transcription.completed, item_id:', event.item_id, 'transcript:', event.transcript?.substring(0, 50));
        let { item, delta } = handler(event);
        console.log('[GA DEBUG] transcription processed, item:', !!item, 'audio length:', item?.formatted?.audio?.length, 'transcript:', item?.formatted?.transcript?.substring(0, 50));
        
        // GA API fix: if item was not found (GA doesn't fire conversation.item.created for user items),
        // create the user item manually from the transcription event
        if (!item && event.item_id && event.transcript) {
          console.log('[GA DEBUG] Creating user item manually from transcription event');
          const userItem = {
            id: event.item_id,
            type: 'message',
            role: 'user',
          ...
          // Capture user audio from the local inputAudioBuffer
          if (this.inputAudioBuffer && this.inputAudioBuffer.length > 0) {
            userItem.formatted.audio = this.inputAudioBuffer.slice();
            console.log('[GA DEBUG] Captured user audio from inputAudioBuffer, length:', userItem.formatted.audio.length);
          } else if (this.lastCommittedAudioBuffer && this.lastCommittedAudioBuffer.length > 0) {
            // Fallback for push-to-talk mode: inputAudioBuffer was already reset by createResponse()
            userItem.formatted.audio = this.lastCommittedAudioBuffer.slice();
            this.lastCommittedAudioBuffer = new Int16Array(0);
            console.log('[GA DEBUG] Captured user audio from lastCommittedAudioBuffer (push-to-talk), length:', userItem.formatted.audio.length);
          }
          
          // Register the item in the conversation lookup
          this.conversation.itemLookup[userItem.id] = userItem;
          this.conversation.items.push(userItem);
          
          // Reset the input audio buffer for the next turn
          this.inputAudioBuffer = new Int16Array(0);
          
          item = userItem;
          delta = { transcript: event.transcript };
        }
    ...
    // GA API: streaming input transcription deltas
    this.realtime.on(
  ...
  getTurnDetectionType() {
    return this.sessionConfig.audio?.input?.turn_detection?.type || null;
  }
   ...
   * Accepts legacy flat params and maps them to the GA nested structure.
   * @param {SessionResourceType} [sessionConfig]
   */
  updateSession({
    output_modalities = void 0,
    modalities = void 0,
    instructions = void 0,
    voice = void 0,
    ...
    // Support both legacy 'modalities' and new 'output_modalities'
    const effectiveModalities = output_modalities || modalities;
    if (effectiveModalities !== void 0 && effectiveModalities !== null) {
      this.sessionConfig.output_modalities = effectiveModalities;
    }
    instructions !== void 0 && (this.sessionConfig.instructions = instructions);

    // GA API: voice, audio formats, transcription are nested under audio object
    if (!this.sessionConfig.audio) {
    ...
    if (input_audio_format !== void 0) {
      const mappedFormat = this._audioFormatMap[input_audio_format] || input_audio_format;
      this.sessionConfig.audio.input.format = { type: mappedFormat, rate: 24000 };
    }
    if (output_audio_format !== void 0) {
      const mappedFormat = this._audioFormatMap[output_audio_format] || output_audio_format;
      this.sessionConfig.audio.output.format = { type: mappedFormat, rate: 24000 };
    }
    // GA API: input_audio_transcription → audio.input.transcription
    if (input_audio_transcription !== void 0) {
      this.sessionConfig.audio.input.transcription = input_audio_transcription;
    }
    // GA API: turn_detection → audio.input.turn_detection
    ...
    // Support both legacy 'max_response_output_tokens' and new 'max_output_tokens'
    // const effectiveMaxTokens = max_output_tokens || max_response_output_tokens;
    // if (effectiveMaxTokens !== void 0 && effectiveMaxTokens !== null) {
    //   this.sessionConfig.max_response_output_tokens = effectiveMaxTokens;
    // }
    ...
    // Build GA-compliant session payload
    const session = { ...this.sessionConfig };
    session.type = 'realtime';
    session.tools = useTools;
    // Remove ALL legacy flat keys that have been moved/renamed in GA
    ...
    delete session.max_output_tokens;
  ...
  createResponse() {
    if (
      ...
      // GA API fix: save committed audio for transcription handler fallback
      this.lastCommittedAudioBuffer = this.inputAudioBuffer.slice();
      this.inputAudioBuffer = new Int16Array(0);
    }
