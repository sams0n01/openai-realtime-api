...
export class RealtimeConversation {
  ...
  /**
   * Trims leading/trailing near-silence from PCM16 audio.
   * @param {Int16Array} samples
   * @param {{threshold?: number, frameSize?: number}} [opts]
   * @returns {Int16Array}
   */
  _trimPcmSilence(samples, opts = {}) {
    if (!(samples instanceof Int16Array) || !samples.length) {
      return new Int16Array(0);
    }
    const threshold = opts.threshold ?? 0.012;
    const frameSize = opts.frameSize ?? 240; // 10ms at 24kHz
    let start = 0;
    let end = samples.length;

    const framePeak = (from, to) => {
      let peak = 0;
    ...
    while (start < end) {
      const next = Math.min(end, start + frameSize);
      if (framePeak(start, next) >= threshold) break;
      start = next;
    }
    while (end > start) {
      const prev = Math.max(start, end - frameSize);
      if (framePeak(prev, end) >= threshold) break;
      end = prev;
    }
    return start >= end ? new Int16Array(0) : samples.slice(start, end);
  }
  ...
  EventProcessors = {
    ...
    'input_audio_buffer.speech_stopped': (event, inputAudioBuffer) => {
      ...
      if (inputAudioBuffer) {
        ...
        speech.audio = this._trimPcmSilence(
          inputAudioBuffer.slice(startIndex, endIndex),
        );
