...
export class RealtimeConversation {
  ...
  EventProcessors = {
    ...
    'conversation.item.truncated': (event) => {
      ...
      const previousAudioLength = item?.formatted?.audio?.length || 0;
      const previousTranscript = `${item?.formatted?.transcript || ''}`;
      const endIndex = Math.floor(
        (audio_end_ms * this.defaultFrequency) / 1000,
      );
      item.formatted.audio = item.formatted.audio.slice(0, endIndex);
      if (previousTranscript.length && previousAudioLength > 0) {
        const safeRatio = Math.max(0, Math.min(1, endIndex / previousAudioLength));
        const transcriptIndex = Math.floor(previousTranscript.length * safeRatio);
        item.formatted.transcript = previousTranscript.slice(0, transcriptIndex).trimEnd();
      } else {
